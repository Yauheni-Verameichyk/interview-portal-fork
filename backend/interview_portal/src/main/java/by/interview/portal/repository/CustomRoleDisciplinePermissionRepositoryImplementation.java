...
public class CustomRoleDisciplinePermissionRepositoryImplementation
	...
	public List<RoleDisciplinePermission> findRoleDisciplinePermissions(
		...
		Query query = entityManager.createNativeQuery(generateSQLQueryString(roleDisciplinesMap, iterationsCount),
	...
	private String generateSQLQueryString(Map<Role, List<Discipline>> roleDisciplinesMap, Integer iterationsCount) {
		String sql = "";
		for (Map.Entry<Role, List<Discipline>> entry : roleDisciplinesMap.entrySet()) {
			...
			if (lDisciplines == null || lDisciplines.isEmpty()) {
				...
				for (int i = 0; i < lDisciplines.size(); i++) {
					String op = iterationsCount == 0 ? "" : " OR ";
					sql += op + "(rdps.role_id = :" + role;
					sql += " AND " + "rdps.discipline_id = :" + lDisciplines.get(i).getName().replaceAll("\\s+", "")
							+ ")";
					iterationsCount++;
				}
			}
	...
	private void setQueryParameters(Map<Role, List<Discipline>> roleDisciplinesMap, Integer iterationsCount,
			Query query) {
		for (Map.Entry<Role, List<Discipline>> entry : roleDisciplinesMap.entrySet()) {
			Role role = entry.getKey();
			List<Discipline> lDisciplines = entry.getValue();
