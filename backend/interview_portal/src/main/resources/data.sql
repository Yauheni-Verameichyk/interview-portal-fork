-- FUNCTION: public.select_specified_time_by_discipline(timestamp without time zone, timestamp without time zone, bigint)
...
join users_roles_disciplines on users_roles_disciplines.discipline_id = for_discipline_id
where not (specified_time.end_time <= range_start OR specified_time.start_time >= range_end)
	LOOP   
